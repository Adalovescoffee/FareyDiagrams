...
# ╔═╡ 5ea2574d-9b8a-4e3a-adc7-9dafe7fbc61a
println(length(intersect([5,1,2],[1,2,4])) == 2)
...
# ╔═╡ 867236cd-729c-4dab-9178-dba3902b2436
FareyGraph(1)[3]
...
# ╔═╡ e775b2df-349f-468b-9592-09ba91ca693a


vertex_neighbors = [[v; neighbors(FareyGraph(2)[1], v)] for v in FareyGraph(2)[3]]

# ╔═╡ 040f0055-f8e2-416a-a1d8-f137db7a6328
function triangleF(n) #gives the indexes of the newer triangles!
	M = FareyGraph(n)[3]
	if n == 1 
		return M 
	else 
...
# ╔═╡ 8246619f-23ef-4f49-bc89-6606093178dc
triangleF(2)
...
# ╔═╡ 45d5eec0-67eb-4180-80f4-d439bf122e07
triangleF(2)
...
function angle(a::Set{Rational{Int}}) # gives the angle
	v = first(a)
...
function equi_angle(a::Set{Rational{Int}}) #gives the angle but if it were to be equidistant
	v = first(a)
	if v ∈ [Rational(-1,0),Rational(1,0)]
		return π
	elseif v>=0 && v<=1
...
equitopograph(1,FareyGraph(1))[1]

# ╔═╡ fc843fa7-0fd5-4a40-b1cb-09e4d1af9d90
equitopograph(1,FareyGraph(1))

# ╔═╡ 21fc5e84-84e3-40e7-b0ed-ebb677a5708a
function topograph(m::Int,G)
	T = []
	DictG = G[2]
	for i in range(1,m)
		for t in triangleF(i) #loops over every new triangle 
			p1_x = my_radius * cos(angle(DictG[t[1]]))
			p1_y = my_radius * sin(angle(DictG[t[1]]))
			p2_x = my_radius * cos(angle(DictG[t[2]]))
			p2_y = my_radius * sin(angle(DictG[t[2]]))
			p3_x = my_radius * cos(angle(DictG[t[3]]))
			p3_y = my_radius * sin(angle(DictG[t[3]]))
...
function plot_farey_graph_with_edges2equi(m::Int,G)
	...
	x_p = my_radius*cos(π)
	y_p = my_radius*sin(π)
	c0 = Luxor.Point(x_0,y_0)
	cp = Luxor.Point(x_p,y_p)
	line(c0,cp,:stroke)
    i = 1
